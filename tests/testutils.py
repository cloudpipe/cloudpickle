...
import psutil
from cloudpickle import dumps, CloudPickleConfig, DEFAULT_CONFIG, hash_dynamic_classdef
...
CONFIG_REGISTRY = {
    ...
    "hashed_classdef": CloudPickleConfig(id_generator=hash_dynamic_classdef),
    "no_id_generator": CloudPickleConfig(id_generator=None),
}


def get_config(config_id):
    """Retrieve CloudPickleConfig by string identifier

    Args:
        config_id: String identifier for the config

    Returns:
        CloudPickleConfig instance

    Raises:
        ValueError: If config_id is not in registry
    """
    if config_id not in CONFIG_REGISTRY:
        raise ValueError(
            f"Unknown config: {config_id}. "
            f"Available: {list(CONFIG_REGISTRY.keys())}"
        )
    return CONFIG_REGISTRY[config_id]


def assert_isinstance_semantics(config_id, original_type, depickled):
    """Assert that depickled instance maintains isinstance semantics with original"""
    if not get_config(config_id).id_generator:
        return
...
def dumps_with_config(obj, protocol, config_id="default"):
    return dumps(obj, protocol=protocol, config=get_config(config_id))


def subprocess_pickle_string(
    input_data, protocol=None, timeout=TIMEOUT, add_env=None, config_id="default"
):
    ...
    pickle_string = dumps_with_config(
        input_data, protocol=protocol, config_id=config_id
    )
    try:
        comm_kwargs = {}
...
def pickle_echo(stream_in=None, stream_out=None, protocol=None, config_id="default"):
    ...
    repickled_bytes = dumps_with_config(obj, protocol=protocol, config_id=config_id)
    stream_out.write(repickled_bytes)
    stream_out.close()


def call_func(payload, protocol, config_id):
    ...
    try:
        ...
        result = e
    return dumps_with_config(result, protocol=protocol, config_id=config_id)


class _Worker:
    def __init__(self, protocol=None, config_id="default"):
    ...
    def run(self, func, *args, **kwargs):
        ...
        input_payload = dumps_with_config(
            (func, args, kwargs), protocol=self.protocol, config_id=self.config_id
        )
        result_payload = self.pool.submit(
            call_func, input_payload, self.protocol, self.config_id
        ).result()
...
@contextmanager
def subprocess_worker(protocol=None, config_id="default"):
    worker = _Worker(protocol=protocol, config_id=config_id)
    yield worker
    worker.close()
...
if __name__ == "__main__":
    ...
    config_id = sys.argv[sys.argv.index("--config") + 1]
    pickle_echo(protocol=protocol, config_id=config_id)
